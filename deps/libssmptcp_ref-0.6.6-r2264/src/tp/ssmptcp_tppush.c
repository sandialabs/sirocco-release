...
int ssmptcp_tppush(ssm_Itp vtp, ssm_Il txs, ssm_Haddr vaddr, ssm_Il heads, ssm_Il mmrs, ssm_Il mds)
{
  ...
  while(txi->next(txi))
  {
    ...
    ssmptcp_tx_init(tx, (conn->next_id)++, 0, mmr->mr, mdi->next(mdi), SSMPTCP_CALL_PUSH, 0, txi->cur(txi));
